...
const Scutter = ({ subjectImage, subjectName, syllabus, rating, handleOnPress, enrollers }) => {
  ...
  return (
    <TouchableOpacity onPress={handleOnPress} style={styles.cardContainer}>
      ...
      <View style={styles.cardTextContainer}>
        <Text style={styles.subjectNameName}>{subjectName}</Text>
        <View style={styles.ratingContainer}>
          <Ionicons name="star" size={16} color="gray" />
          <Text style={styles.ratingText}>{rating}</Text>
          <Text style={styles.ratingText}>{'('+enrollers+')'}</Text>
        </View>
